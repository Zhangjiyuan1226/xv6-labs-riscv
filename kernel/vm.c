...
void
userkvmmap(pagetable_t kpagetable,uint64 va, uint64 pa, uint64 sz, int perm)
{
...
void
uvmfree2(pagetable_t pagetable_t, uint64 va, uint npages){
  if(npages > 0)
    uvmunmap(pagetable_t, va, npages, 1);
